...
const DividendApp = (function () {
    ...
    async function loadStocks(forceRefresh = false) {
        ...
        try {
            ...
            // Handle seeding or needs-init state
            if (data.status === 'needs_initialization' || data.isSeeding) {
                if (elements.cardStack) {
                    elements.cardStack.innerHTML = `
                        <div style="text-align: center; padding: 48px; color: var(--text-muted);">
                            <!-- Icon: Database/Server -->
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin-bottom: 16px; opacity: 0.8;">
                                <ellipse cx="12" cy="5" rx="9" ry="3"></ellipse>
                                <path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"></path>
                                <path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"></path>
                            </svg>
                            <div style="margin-bottom: 8px; font-weight: bold; font-family: var(--font-display);">Initialize Data</div>
                            <div style="font-size: 0.875rem;">Load latest S&P 500 dividend data</div>
                            <button onclick="DividendApp.refresh()" style="margin-top: 24px; padding: 12px 24px; background: var(--accent-primary); border: none; border-radius: 8px; color: var(--bg-primary); cursor: pointer; font-weight: 700; font-family: var(--font-body);">
                                Update Data Now
                            </button>
                        </div>
                    `;
    ...
    /**
     * Checks data age and shows update button if needed.
     */
    function checkDataFreshness(fetchedAt) {
        ...
        if (elements.updateDataBtn) {
            // Show update button if data is older than 7 days
            if (diffDays > 7) {
    ...
    async function refresh() {
        ...
        if (elements.updateDataBtn) {
            ...
            // Hide button after successful update (assuming fresh data)
            elements.updateDataBtn.style.display = 'none';
            elements.updateDataBtn.innerHTML = `
